...
class DjangoYnhTestCase(HtmlAssertionMixin, TestCase):
    ...
    def test_settings(self):
        ...
        self.assertTrue(
            str(settings.LOG_FILE_PATH).endswith('/local_test/var_log_django-fritzconnection.log'),
        ...
        self.assertTrue(log_filename.endswith('/local_test/var_log_django-fritzconnection.log'), log_filename)
        self.assertEqual(
    ...
    def test_login_happy_path(self):
        ...
        with self.assertLogs('django_yunohost_integration') as logs, NoFritzBoxConnection():
            response = self.client.get(
                path='/app_path/admin/management/manage-host-wan-access-via-host-groups/',
                HTTP_YNH_USER='test',
                HTTP_AUTH_USER='test',
        ...
        self.assertEqual(response.status_code, 200, repr(response))
        self.assertTemplateUsed(response, template_name='djfritz/group_management.html')
        self.assert_html_parts(
            response,
            parts=(
                '<strong>test</strong>',
                '<a href="/app_path/admin/diagnose/test-fritzbox-connection/">Test FritzBox connection</a>',
                '<a href="/app_path/admin/host-information/get-raw-mesh-topology/">Get raw mesh topology</a>',
            ),
        )
        self.assertEqual(
            logs.output,
